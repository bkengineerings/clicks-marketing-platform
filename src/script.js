...
function generateRansomTitle() {
    ...
    words.forEach(word => {
        ...
        word.split('').forEach(char => {
            ...
            span.className = 'char inline-block px-2 py-1 md:px-4 md:py-2 shadow-2xl border border-white/10 transition-all duration-500 hover:scale-110 cursor-pointer';
...
function fillSlider() {
    ...
    const fillLimit = window.innerWidth * 5; // Extra breed voor zoom
    let currentWidth = 0;
    ...
    while (currentWidth < fillLimit) {
        images.forEach(src => {
            ...
            div.innerHTML = `<img src="${src}" class="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700">`;
